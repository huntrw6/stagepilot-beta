...
async def test_demo_plan_can_drive_real_propresenter_plugin() -> None:
    ...
    async with app.router.lifespan_context(app):
        ...
        outcome = await runtime.state_service.dispatch(
            ActionName.START_NEXT, source="test"
        )
        ...
        restart = await runtime.state_service.dispatch(
            ActionName.RESTART_CURRENT, source="test"
        )
        ...
        stopped = await runtime.state_service.dispatch(
            ActionName.STOP_TIMER, source="test"
        )
